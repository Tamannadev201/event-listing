...
export const registerUser = async (req, res) => {
  try {
    ...
    const newUser = new User({
      name, email, password: hashedPassword, address, phoneNumber, country, division, district
    });
...
export const loginUser = async (req, res) => {
  try {
    ...
    res.status(200).json({ msg: 'Login successful!', token, user: { name: user.name, email: user.email } });
  } catch (error) {
    console.error(error);
    res.status(500).json({ msg: 'Something went wrong!' });
  }
